...
const FlightExplore = () => {
  ...
  useEffect(() => {
    ...
    const minor = parseInt(searchParams.get('minor') || '0');
    ...
    // Find matching suggestions for from and to
    const fromSuggestion = suggestions.find(s => s.name === from);
    const toSuggestion = suggestions.find(s => s.name === to);

    if (fromSuggestion) {
      departureSuggest.setInput(fromSuggestion);
      departureSuggest.setSearchText(fromSuggestion.name);
    }
    if (toSuggestion) {
      arrivalSuggest.setInput(toSuggestion);
      arrivalSuggest.setSearchText(toSuggestion.name);
    }
  ...
  return (
    <>
      <div className="px-8 w-full flex flex-col">
        <div className="mt-10">
          {/* Phần hiển thị thông tin tìm kiếm */}
          <div className="bg-white p-6 rounded-xl shadow-lg border-2 border-gray-200">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div className="flex flex-col gap-3 w-full md:w-auto">
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-lg text-[#1A1D1F]">{departureSuggest.input?.name || ''}</span>
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M5 12H19M19 12L12 5M19 12L12 19" stroke="#605DEC" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                    <span className="font-semibold text-lg text-[#1A1D1F]">{arrivalSuggest.input?.name || ''}</span>
                  </div>
                </div>
                <div className="flex flex-wrap items-center gap-4 text-[#6F767E]">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="w-1 h-1 bg-[#6F767E] rounded-full"></div>
                  <div className="flex items-center gap-2">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <span>{options.adult} người lớn{options.minor > 0 ? `, ${options.minor} trẻ em` : ''}</span>
                  </div>
                </div>
              </div>
              <button
                onClick={() => setShowSearchForm(!showSearchForm)}
                className="flex items-center gap-2 px-4 py-2 text-[#605DEC] hover:text-[#4B48BF] font-medium transition-colors rounded-lg hover:bg-[#F6F6FE]"
              >
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          {/* Form tìm kiếm */}
          {showSearchForm && (
            <div className="mt-4 bg-white p-6 rounded-xl shadow-lg border-2 border-gray-200 transition-all duration-300">
        ...
        <div className="mt-16">
          <FlightChoose searchData={searchData} />
        </div>
      </div>
    </>
  );
