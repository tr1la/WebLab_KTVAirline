...
graph TD
    ...
    subgraph Exposure
        ...
        N["Jasper legacyJspServlet: *.jsp / *.JSP"]
    end
...
| JSP execution mapping   | `JspServlet`, `ServletRegistrationBean`, `*.jsp`, `*.JSP`                      | `LegacyUploadWebResourceConfig.legacyJspServlet()`                                      | `.JSP` upload có thể được Jasper compile/execute                        |
...
| `ServletRegistrationBean<>(new JspServlet(), "*.jsp", "*.JSP")` | Route file `.jsp`/`.JSP` sang JSP servlet |
...
Như vậy, với direct JSP impact, sink -> source không dừng ở `WebConfig.addResourceHandler(...)`. Reviewer phải tiếp tục fuzz `JspServlet`, `ServletRegistrationBean`, `DirResourceSet`, `addPreResources`, và dependency `tomcat-embed-jasper` để xác nhận file upload có thể được xử lý như executable servlet resource.
...
| Tìm servlet mapping | `ServletRegistrationBean<>(new JspServlet(), "*.jsp", "*.JSP")` |
...
8. `legacyJspServlet` map cả `*.JSP`, biến blacklist bypass từ "ghi file lạ" thành executable upload.
...
Nếu cần helper, expose instance đã an toàn:
